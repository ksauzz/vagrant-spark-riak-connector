    ...
    cluster.vm.define "#{opt.hostname(index)}".to_sym do |config|
      ...
      if Vagrant.has_plugin?("vagrant-cachier")
        ...
        config.cache.enable :generic, {"wget"  => { cache_dir: "/var/cache/wget" },
                                       "maven" => { cache_dir: "/var/cache/maven" }}
        config.vm.provision "shell", inline: "echo MAVEN_OPTS=-Dmaven.repo.local=/var/cache/maven > /etc/mavenrc"
        config.vm.provision "shell", inline: "chown -R vagrant:vagrant /var/cache/maven"
      end
      ...
      config.vm.provision "shell", path: "install.sh", args: [opt.address(index)]
    end
  end
end
